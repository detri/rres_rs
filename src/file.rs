//! The `file` module contains data structures and helpers for reading RRES files.
use std::error::Error;
use std::fmt::{Display, Formatter, Result as FmtResult};
use std::fs::File;
use std::io::{BufReader, Cursor, Read, Seek, SeekFrom};
use byteorder::{NativeEndian, ReadBytesExt};
use crate::chunks::{ResourceChunk, ResourceChunkData, ResourceChunkInfo};
use crate::errors::RresError;
use crate::ext::ReadCcFour;
use crate::ResourceDataType;
...
impl FileHeader {
    pub fn from_buf_reader(reader: &mut BufReader<File>) -> Result<FileHeader, Box<dyn Error>> {
...
impl CentralDir {
    pub fn get_resource_id(&self, filename: String) -> u32 {
        let mut id: u32 = 0;
...
impl RresFile {
    pub fn load_resource_chunk(&self, rres_id: u32) -> Result<ResourceChunk, Box<dyn Error>> {
    ...
    pub fn load_central_dir(&self) -> Result<CentralDir, Box<dyn Error>> {
